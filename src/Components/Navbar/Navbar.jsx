...
export  function Navbar({bri,home,star,main,mainveg,rot,cart}) {
  ...
  useEffect(()=>{
    ...
    setCartCount(cart?.length || JSON.parse(localStorage.getItem('cxcart'))?.length || 0)
  },[cart])

  useEffect(()=>{
    const handler = (e) => {
      // parenthesize to avoid mixing `??` and `||` without parentheses
      const c = (e?.detail ?? JSON.parse(localStorage.getItem('cxcart'))) || []
